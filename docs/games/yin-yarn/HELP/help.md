...
- 購買遊戲中盤面出現 jackpot gem 即給獎；落 1／2／3／4／5 顆 → 開出的段數不同（越多顆越高段：5 顆→GRAND）。
- jackpot gem 僅於購買遊戲出現。

## ★ owl 貓符區分（易混，依 HELP＋實機圖）
盤面有多顆貓臉 owl，依底色／字樣／出現時機與獎項區分：
- **彩金貓 Jackpot Gem**（金底無字，bg-jp.png）：**基本遊戲**（bought game）落 1~5 顆 → 開 GRAND／MAJOR／SUPER／MINOR／MINI（Grand/Major 累積、Super/Minor/Mini 固定）。help-02。
- **JACKPOT owl**（金底 JACKPOT 字，jackpot-gem.png）：**特色期間**出現，獎值加進該輪 pot 並 ×該輪 upgrade level（credit 獎值，非段位彩金；確切值 {{待確認}}）。help-05／21。
...
- **YINYARN FEATURES**（help-03 觸發頁、help-04 組合、help-05 續）：購買遊戲中依毛線球觸發核心／Boost／Extra／Gem。help-05 原文載明：觸發於 **YARNY 給 8 場、YARNIER 給 9 場**免費遊戲，並在 5 輪上方各放一個 bronze counter；並列出 7 大特色期間會出現的符號（紅／藍／金幣＋Multiplier／Jackpot／Upgrade owl）與 coin 揭獎、counter 升階規則。
- **COIN UPGRADE**（help-06）：紅→藍→金三色幣值階，每次升級提高獎值上下限（HELP 頁列出 +10／+20、+20／+80、+200／+800 credits × bet multiplier；報告展示文字依使用者指示改記為「隱藏數值」）。
- **GEM FEATURE**（help-07；owl 觸發，僅購買遊戲）：由 cut 選 START FEATURE，導向核心／Extra／Boost，並加免費遊戲。
